...
def Movement_Dormant(position, Newposition, material, arrow, health, step_Cost):
...
def Movement_Ready(position,Newposition,material,arrow,health,step_Cost):
...
def Ready(action,position,Newposition,material,arrow,health,step_Cost):

    if action in Movement_actions:
        
        return Movement_Ready(position,Newposition,material,arrow,health,step_Cost)
...
def Dormant(action,position,Newposition,material,arrow,health,step_Cost):

    if action in Movement_actions:

        return Movement_Dormant(position,Newposition,material,arrow,health,step_Cost)
...
def MM_state(action,position,New_position,material,arrow,state,health,step_Cost) : 

    if state == 1 :
        return Ready(action,position,New_position,material,arrow,health,step_Cost)

    if state == 0 : 
        return Dormant(action,position,New_position,material,arrow,health,step_Cost)
                    ...
                    for state in range(2):
                        for health in range(5):
                            ...
                            for action,New_position in actions_position[position_key[position]].items():
                                ...
                                step_Cost = -20
                                if (action == "STAY"): step_Cost = 0
                                
                                state_value = MM_state(action,position,New_position,material,arrow,state,health,step_Cost)
