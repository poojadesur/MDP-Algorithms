...
def Movement_Dormant(position, Newposition, material, arrow, health, colIdx):
    ...
    idx = get_index(3,material,arrow,0,health)
    A[idx][colIdx] -= ((1-prob) * 0.8)

    #action fail and monster R:
    idx = get_index(3,material,arrow,1,health)
...
def Shoot_Dormant (position,material,arrow,health,colIdx):
    ...
    # east
    if(position == 3) : hitprob = 0.9
    # west
    if(position == 4): hitprob = 0.25
...
def Movement_Ready(position,Newposition,material,arrow,health,colIdx):
    ...
    ind = get_index(3, material, arrow, 1, health)
    A[ind][colIdx] -= (1-prob)*0.5

    # if IJ is in center or east
    if(position == 0 or position == 3):
        #MM attack, IJ always fail moves to East, -40 reward # !!!!! WE CHANGED FAIL TO STAY AT YOUR POSITION
        ...
        #MM attack, IJ fail
        ind = get_index(3, material, arrow, 0, health)
        A[ind][colIdx] -= (1-prob) * 0.5
